...
def main(field):
    ...
    dfsum.to_json(f"data\\prod\\{field}\\cuml.json", orient='values', date_format='iso')
    #write_formations()

    if field == "ST": update_pumpInfo(); analyze(pd.read_csv(f'data\\prod\\{field}\\data.csv'),'ST')
    return
...
def move(field):
    ...
    try:
        ...
        print(err)
    if field == 'ST': pd.read_json('data\\prod\\ST\\pumpinfo.json').to_json('../frontend/data/pumpInfo.json',orient='values',date_format='iso')
    return   
...
if __name__ == '__main__':
    #for FIELD in ['ST','ET']: main(field=FIELD); move(field=FIELD); time.sleep(60)
