...
def main():
    ...
    df_to_json_format.to_json("allProductionData.json", orient='values', date_format='iso') #updating loc json file
    dfsum.to_json("cumProd.json", orient='values', date_format='iso')
...
def importDataIwell(start):
    ...
    some = ["Aaron #1"] #test
    updates = []
    #change sorted(st) to some to test
