...
def pl(file,keep):
    ...
    for _,row in df.iterrows():
        well_map[row[well_col]] = row[wellname_col] 
    df = df[stmts]
...
def main():
    ...
    well_map = {str(key): value for key, value in well_map.items()}
    well_map = {key: value for key, value in well_map.items() if value != 'South Texas Total'}
    print(well_map)
    with open('db/econ/well_map.json', 'w') as json_file:
        json.dump(well_map, json_file)
