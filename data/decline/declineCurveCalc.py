...
def main():
    ...
    df_wells = pd.read_json('./data/decline/stWells.json')
...
def declineCurve(name):
    print(name)
    data = pd.read_csv('data/prod/ST/moData.csv')
    ...
    # RUN economics() ======================================================================================
    q_eco, qm_eco, fp_eco, ecoLimit, modelEcoLimit,npVal = economics(q_real, q_model, t_model, qi_est, b_est, Di_est, extr_mo, drop_index)
    ...
    df_final.to_csv(f"../frontend/data/declineCurvesUpdated/{formatted_name}.csv", index=False)
    #------------------^^^^^^^----------------------------------------------------------------#

    # RETURNS 'PARAMATERS'
    return {"qi":qi_est, "D":Di_est, "b":b_est, "extr_mo":extr_mo, "q_sum":q_eco, "qm_sum":qm_eco, "future_prod":fp_eco, "eco_limit":ecoLimit, "end_index":end_index, "model_eco_limit":modelEcoLimit, "np_value": npVal}
