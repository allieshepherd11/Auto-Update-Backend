...
def format_prism_data(df:pd.DataFrame,wells=[]):
    ...
    df = df[['WellName','ProducingMonth','LiquidsProd_BBL','GasProd_MCF']]
    df = df.rename(columns={'WellName':'Well Name','ProducingMonth':'Date','LiquidsProd_BBL':'Oil (BBLS)','GasProd_MCF':'Gas (MCF)'})
...
def main():
    ...
    for well in ptnwells:
        if well in skippnt:continue
        well_params = declineCurve(well,df_ptn_et_prod.copy(),'Gas (MCF)')
...
def declineCurve(name,df,fluidType='Oil (BBLS)'):
    ...
    q_real = df_data[fluidType].values.tolist()
    ...
    if start_at_idx == None:
        start_at_idx = df[:10].reset_index()[fluidType].idxmax()
    ...
    q = actual_df[fluidType].values.tolist() # list of production data
    qi = q[0]
